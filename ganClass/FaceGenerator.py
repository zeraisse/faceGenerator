...
class FaceGenerator:
    ...
    def training_face(self, epochs=10, start_epoch=0):
        ...
        for epoch in range(start_epoch, epochs):
            ...
            # Images : Toutes les 50 époques OU à la dernière époque
            if epoch % 50 == 0 or epoch == epochs - 1:
                self.generator.save_generated_images(epoch, i, self.fixed_noise)
                print(f"--- [SAVE] Images sauvegardées epoch {epoch} ---")

            # Modèles : Toutes les 20 époques OU à la dernière époque
            if epoch % 20 == 0 or epoch == epochs - 1:
                self.save_models(epoch)
                print(f"--- [SAVE] Modèles sauvegardés epoch {epoch} ---")
    ...
    def load_latest_models(self, output_dir="saved_models"):
        ...
        Retourne le numéro de la dernière époque sauvegardée, ou 0 si aucun modèle trouvé.
        """
        if not os.path.exists(output_dir):
        ...
        try:
            ...
            # On retourne last_epoch + 1 pour commencer l'entrainement à l'époque suivante
            return last_epoch + 1 
